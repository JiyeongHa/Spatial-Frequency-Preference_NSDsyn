...
def _find_min_and_max(img_min_max, stim_df=):
...
# each voxel's ecc -> # return voxels that have prf centers outside of stimuli
def _check_voxel_ecc_range(min_ecc=min_ecc, max_ecc=max_ecc):
    pass
    return voxel_df

def

# remove the voxels from dataframe

def drop_voxels_outside_ecc_range(df):
    groupby_col = ['voxel']
